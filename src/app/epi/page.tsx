...
import { BarChart3, Table as TableIcon, RefreshCw, Search, Syringe } from 'lucide-react'
import {
...
export default function EpiPage() {
  ...
  const totals = filtered.reduce((a, c) => ({
    total_epi: a.total_epi + Number(c.total_epi || 0),
    total_persons: a.total_persons + Number(c.total_persons || 0),
  }), { total_epi: 0, total_persons: 0 })

  const chartData = filtered.map(d => ({
    name: d.hosname?.replace('รพ.สต.', '').replace('โรงพยาบาลส่งเสริมสุขภาพตำบล', '') || d.hospcode,
    'ครั้งที่ฉีด': Number(d.total_epi || 0),
    'คนที่ได้รับ': Number(d.total_persons || 0),
  }))
  ...
  return (
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '24px', paddingBottom: '40px' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '16px' }}>
        <div>
          <h1 style={{ fontSize: '24px', fontWeight: 700, color: 'var(--text-primary)', display: 'flex', alignItems: 'center', gap: '10px' }}>
            <Syringe color="#06b6d4" size={28} /> สร้างเสริมภูมิคุ้มกันโรค EPI (ปีงบ 69)
          </h1>
          <p style={{ fontSize: '14px', color: 'var(--text-muted)', marginTop: '4px' }}>จำนวนครั้งการฉีดวัคซีน แยกตามหน่วยบริการ ตั้งแต่ 1 ต.ค. 2568</p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px' }}>
        {[
          { label: 'ครั้งที่ฉีดวัคซีน', value: totals.total_epi, bg: '#cffafe', ic: '#0891b2' },
          { label: 'ผู้ที่ได้รับวัคซีน (คนไม่ซ้ำ)', value: totals.total_persons, bg: '#e0f2fe', ic: '#0369a1' },
        ].map(w => (
      ...
      <div style={{ background: 'var(--bg-card)', padding: '24px', borderRadius: '16px', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
        <h2 style={{ fontSize: '18px', fontWeight: 700, marginBottom: '20px', color: 'var(--text-primary)', display: 'flex', alignItems: 'center', gap: '8px' }}>
          <BarChart3 size={20} color="var(--accent)" /> กราฟแสดงจำนวนการฉีดวัคซีนแยกตามหน่วยบริการ
        </h2>
        <div style={{ height: '380px' }}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={chartData} margin={{ top: 20, right: 20, left: 10, bottom: 70 }}>
              ...
              <Bar dataKey="ครั้งที่ฉีด" fill="#06b6d4" radius={[4, 4, 0, 0]} barSize={20}>
                <LabelList dataKey="ครั้งที่ฉีด" position="top" style={{ fontSize: '10px', fill: 'var(--text-muted)' }} />
              </Bar>
              <Bar dataKey="คนที่ได้รับ" fill="#0891b2" radius={[4, 4, 0, 0]} barSize={20}>
                <LabelList dataKey="คนที่ได้รับ" position="top" style={{ fontSize: '10px', fill: 'var(--text-muted)' }} />
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
            <thead style={{ background: 'var(--bg-primary)' }}>
              <tr>
                {['รหัส', 'หน่วยบริการ', 'ครั้งที่ฉีดวัคซีน', 'ผู้ที่ได้รับวัคซีน (คน)'].map(h => (
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={row.hospcode} style={{ background: i % 2 === 0 ? 'transparent' : 'rgba(0,0,0,0.01)' }}
                  ...
                  <td style={{ padding: '12px 20px', fontSize: '14px', textAlign: 'center', fontWeight: 600, color: '#0891b2' }}>{Number(row.total_epi || 0).toLocaleString()}</td>
                  <td style={{ padding: '12px 20px', fontSize: '14px', textAlign: 'center', fontWeight: 600, color: '#0369a1' }}>{Number(row.total_persons || 0).toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
            <tfoot style={{ background: 'var(--bg-primary)', fontWeight: 700 }}>
              <tr>
                ...
                <td style={{ padding: '14px 20px', textAlign: 'center', fontSize: '16px', color: '#0891b2' }}>{totals.total_epi.toLocaleString()}</td>
                <td style={{ padding: '14px 20px', textAlign: 'center', fontSize: '16px', color: '#0369a1' }}>{totals.total_persons.toLocaleString()}</td>
              </tr>
            </tfoot>
          </table>
        </div>
