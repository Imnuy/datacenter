...
export default function HCVNoVaccinePage() {
    ...
    const fetchMonthlyData = async () => {
        ...
        try {
            const res = await fetch(`/api/report/monthly?year=${selectedYear}&type=hcv`)
            const json = await res.json()
            if (json.success) {
    ...
    return (
        <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '24px', paddingBottom: '40px' }}>
            ...
            {/* Summary Widget */}
            <div style={{ background: 'white', padding: '20px', borderRadius: '16px', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)', display: 'flex', alignItems: 'center', gap: '16px', maxWidth: '400px' }}>
