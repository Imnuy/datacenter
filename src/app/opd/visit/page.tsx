...
import { BarChart3, Table as TableIcon, Users, RefreshCw, Search, Hospital } from 'lucide-react'
import {
...
export default function OPDVisitPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      const res = await fetch('/api/opd?type=visit-summary')
  ...
  return (
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '24px', paddingBottom: '40px' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '16px' }}>
        <div>
          <h1 style={{ fontSize: '24px', fontWeight: 700, color: 'var(--text-primary)', display: 'flex', alignItems: 'center', gap: '10px' }}>
            <Hospital color="var(--accent)" size={28} /> การให้บริการ OPD (ปีงบ 69)
          </h1>
          <p style={{ fontSize: '14px', color: 'var(--text-muted)', marginTop: '4px' }}>จำนวนครั้งและจำนวนผู้รับบริการ OPD แยกตามหน่วยบริการ ตั้งแต่ 1 ต.ค. 2568</p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '16px' }}>
        {[
          { label: 'จำนวนครั้งการรับบริการ', value: totals.total_visit, color: '#dbeafe', icon: '#2563eb' },
          { label: 'จำนวนผู้รับบริการ (คนไม่ซ้ำ)', value: totals.total_patients, color: '#dcfce7', icon: '#16a34a' },
        ].map(w => (
          <div key={w.label} style={{ background: 'white', padding: '20px', borderRadius: '16px', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)', display: 'flex', alignItems: 'center', gap: '16px' }}>
            <div style={{ width: '48px', height: '48px', borderRadius: '12px', background: w.color, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <Users size={24} color={w.icon} />
            </div>
            <div>
      ...
      <div style={{ background: 'var(--bg-card)', padding: '24px', borderRadius: '16px', border: '1px solid var(--border)', boxShadow: 'var(--shadow-sm)' }}>
        <h2 style={{ fontSize: '18px', fontWeight: 700, marginBottom: '20px', color: 'var(--text-primary)', display: 'flex', alignItems: 'center', gap: '8px' }}>
          <BarChart3 size={20} color="var(--accent)" /> กราฟแสดงจำนวนครั้งบริการแยกตามหน่วยบริการ
        </h2>
        <div style={{ height: '380px' }}>
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={chartData} margin={{ top: 20, right: 20, left: 10, bottom: 70 }}>
              ...
              <Bar dataKey="ครั้ง" fill="#3b82f6" radius={[4, 4, 0, 0]} barSize={18}>
                <LabelList dataKey="ครั้ง" position="top" style={{ fontSize: '10px', fill: 'var(--text-muted)' }} />
              </Bar>
              <Bar dataKey="คน" fill="#10b981" radius={[4, 4, 0, 0]} barSize={18}>
                <LabelList dataKey="คน" position="top" style={{ fontSize: '10px', fill: 'var(--text-muted)' }} />
              </Bar>
            </BarChart>
          </ResponsiveContainer>
        </div>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
            <thead style={{ background: 'var(--bg-primary)' }}>
              <tr>
                {['รหัส', 'หน่วยบริการ', 'จำนวนครั้ง', 'จำนวนคน (ไม่ซ้ำ)'].map(h => (
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={4} style={{ padding: '40px', textAlign: 'center', color: 'var(--text-muted)' }}>ไม่พบข้อมูล</td></tr>
              ) : filtered.map((row, i) => (
                <tr key={row.hospcode} style={{ background: i % 2 === 0 ? 'transparent' : 'rgba(0,0,0,0.01)' }}
                  ...
                  <td style={{ padding: '12px 20px', fontSize: '14px', textAlign: 'center', fontWeight: 600, color: '#2563eb' }}>{Number(row.total_visit || 0).toLocaleString()}</td>
                  <td style={{ padding: '12px 20px', fontSize: '14px', textAlign: 'center', fontWeight: 600, color: '#16a34a' }}>{Number(row.total_patients || 0).toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
            <tfoot style={{ background: 'var(--bg-primary)', fontWeight: 700 }}>
              <tr>
                ...
                <td style={{ padding: '14px 20px', textAlign: 'center', fontSize: '16px', color: '#2563eb' }}>{totals.total_visit.toLocaleString()}</td>
                <td style={{ padding: '14px 20px', textAlign: 'center', fontSize: '16px', color: '#16a34a' }}>{totals.total_patients.toLocaleString()}</td>
              </tr>
            </tfoot>
          </table>
        </div>
