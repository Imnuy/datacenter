...
import {
  ...
  Search, Pill, ChevronDown, ChevronUp, MapPin
} from 'lucide-react'
import {
...
interface DrugRow { hospcode: string; hosname: string; didstd: string; dname: string; total: number; rn: number }
interface DistrictRow { didstd: string; dname: string; total: number }
...
function CustomTooltip({ active, payload }: any) {
  ...
  return (
    <div style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: '12px', padding: '10px 16px', boxShadow: 'var(--shadow-md)', maxWidth: '260px' }}>
      <div style={{ fontWeight: 700, fontSize: '13px', color: 'var(--text-primary)', lineHeight: 1.4 }}>{d.dname || d.name}</div>
      <div style={{ fontSize: '11px', color: 'var(--text-muted)', marginTop: '3px', fontFamily: 'monospace' }}>{d.didstd}</div>
      <div style={{ fontSize: '13px', color: 'var(--text-muted)', marginTop: '4px' }}>{Number(payload[0].value).toLocaleString()} ครั้ง</div>
    </div>
  )
}
...
function DistrictTab() {
  ...
  const fetch = async (y = year) => {
    ...
    try {
      const res = await window.fetch(`/api/opd?type=top-drug-district&year=${y}`)
  ...
  const truncate = (s: string, n = 18) => s && s.length > n ? s.slice(0, n) + '…' : s
  const chartData = data.map((d, i) => ({ name: truncate(d.dname || d.didstd), didstd: d.didstd, dname: d.dname, total: Number(d.total), rank: i + 1 }))
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
          ...
          <div style={{ background: 'var(--bg-card)', borderRadius: '16px', border: '1px solid var(--border)', padding: '24px', boxShadow: 'var(--shadow-sm)' }}>
            <h2 style={{ fontSize: '15px', fontWeight: 700, marginBottom: '18px', color: 'var(--text-primary)', display: 'flex', alignItems: 'center', gap: '8px' }}>
              ...
              กราฟ 10 อันดับยา — ภาพรวมอำเภอ ปีงบ {year}
            </h2>
            <div style={{ height: '360px' }}>
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={chartData} layout="vertical" margin={{ left: 10, right: 60, top: 5, bottom: 5 }}>
                  ...
                  <YAxis type="category" dataKey="name" width={160} style={{ fontSize: '11px', fontWeight: 600 }} />
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead style={{ background: 'var(--bg-primary)' }}>
                <tr>
                  ...
                  <th style={{ padding: '10px 16px', fontSize: '12px', fontWeight: 600, color: 'var(--text-muted)', textAlign: 'left' }}>ชื่อยา</th>
                  ...
                  return (
                    <tr key={i} style={{ borderTop: '1px solid var(--border)', background: i % 2 === 0 ? 'transparent' : 'rgba(0,0,0,0.015)' }}>
                      ...
                      <td style={{ padding: '12px 16px' }}>
                        <div style={{ fontWeight: 600, fontSize: '13px', color: 'var(--text-primary)', marginBottom: '3px' }}>{d.dname || d.name}</div>
                        <div style={{ fontFamily: 'monospace', fontSize: '11px', color: 'var(--text-muted)' }}>{d.didstd}</div>
                      </td>
...
function UnitTab() {
  ...
  const fetch = async (y = year, hosp = selectedHosp) => {
    setLoading(true); setError('')
    try {
      const url = `/api/opd?type=top-drug&year=${y}${hosp ? `&hospcode=${hosp}` : ''}`
  ...
  const grouped = useMemo(() => {
    const map: Record<string, { hospcode: string; hosname: string; drugs: DrugRow[] }> = {}
    data.forEach(row => {
      if (!map[row.hospcode]) map[row.hospcode] = { hospcode: row.hospcode, hosname: row.hosname, drugs: [] }
      map[row.hospcode].drugs.push(row)
    })
  ...
  const chartData = selectedHosp && grouped[0]
    ? grouped[0].drugs.map((d, i) => ({ name: truncate(d.dname || d.didstd), didstd: d.didstd, dname: d.dname, total: Number(d.total), rank: i + 1 }))
    : []
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      {selectedHosp && chartData.length > 0 && (
        <div style={{ background: 'var(--bg-card)', borderRadius: '16px', border: '1px solid var(--border)', padding: '24px', boxShadow: 'var(--shadow-sm)' }}>
          <h2 style={{ fontSize: '15px', fontWeight: 700, marginBottom: '18px', color: 'var(--text-primary)', display: 'flex', alignItems: 'center', gap: '8px' }}>
            <BarChart3 size={17} color="var(--accent)" />
            {grouped[0]?.hosname} — 10 อันดับยา ปีงบ {year}
          </h2>
      ...
      {loading ? (
        ...
        grouped.map(hosp => {
          ...
          const maxTotal = Number(hosp.drugs[0]?.total) || 1
          return (
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                  <thead style={{ background: 'var(--bg-primary)' }}>
                    <tr>
                      ...
                      <th style={{ padding: '10px 16px', fontSize: '12px', fontWeight: 600, color: 'var(--text-muted)' }}>ชื่อยา</th>
                      <th style={{ padding: '10px 16px', fontSize: '12px', fontWeight: 600, color: 'var(--text-muted)', textAlign: 'right' }}>จำนวน (ครั้ง)</th>
                      ...
                      return (
                        <tr key={i} style={{ borderTop: '1px solid var(--border)', background: i % 2 === 0 ? 'transparent' : 'rgba(0,0,0,0.015)' }}>
                          ...
                          <td style={{ padding: '11px 16px' }}>
                            <div style={{ fontWeight: 600, fontSize: '13px', color: 'var(--text-primary)', marginBottom: '3px' }}>{d.dname || d.didstd}</div>
                            <div style={{ fontFamily: 'monospace', fontSize: '11px', color: 'var(--text-muted)' }}>{d.didstd}</div>
                          </td>
...
/* ═══════════════════════════════════════════════════════════════
   Main Page with Tabs
═══════════════════════════════════════════════════════════════ */
export default function OpdDrugPage() {
    ...
    <div className="animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '24px', paddingBottom: '40px' }}>
      ...
      <div>
        <h1 style={{ fontSize: '22px', fontWeight: 700, color: 'var(--text-primary)', display: 'flex', alignItems: 'center', gap: '10px' }}>
          <Pill color="#6366f1" size={26} />
          10 อันดับยาที่จ่ายสูงสุด (OPD)
        </h1>
        <p style={{ fontSize: '13px', color: 'var(--text-muted)', marginTop: '4px' }}>
          จากแฟ้ม drug_opd — รหัสยามาตรฐาน (didstd) จำแนกตามปีงบประมาณ
        </p>
      </div>
